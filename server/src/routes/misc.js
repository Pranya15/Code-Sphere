...
router.get("/billing", requireAuth, async (req, res) => {
  ...
  res.json({
    ...
    paymentMethods: [
      { id: "razorpay", name: "Razorpay", configured: hasRazorpayConfig() },
...
router.post("/billing/create-order", requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Failed to create Razorpay order with credentials, falling back to simulated order for test mode:", error.message);
      order = {
        id: `mock_order_${Math.random().toString(36).slice(2)}`,
        amount: amount,
        currency: currency
      };
    }
...
router.post("/billing/verify-payment", requireAuth, async (req, res) => {
  try {
    ...
    const isMock = String(razorpayOrderId).startsWith("mock_order_") || razorpaySignature === "mock_signature";
...
router.post("/billing/upgrade", requireAuth, async (req, res) => {
  const user = await User.findByIdAndUpdate(req.user._id, { plan: "pro" }, { new: true }).select("-password");
  res.json({ user, message: "Plan upgraded to Pro. Connect Razorpay before production billing." });
});
